...
fn main() {
    ...
    // FnOnce - замыкание может быть вызвано только один раз и может перемещать захваченное значение из своего тела
    // FnMut - могут вызываться более одного раза, не перемещают захваченные значения из своего тела но могут их изменять
    // Fn - могут вызываться несколько раз одновлеменно, ни чего не меремещают из себя и ни чего не модифицируют из того что захватили снаружи либо вообще ни чего не захватывают
}
